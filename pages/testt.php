...
if (isset($_POST['send'])) {
 ...
 // إذا كانت قيمة status فارغة، تعيينها إلى '1'
    ...
    $add->bindValue(':height', $height);
        ...
        if ($add->execute()) {
            echo "<script>alert('تم الإدخال.');</script>";
            header("Location: page_customer_service.php"); // إعادة توجيه المستخدم
            exit;
        }
